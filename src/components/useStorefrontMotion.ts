...
/**
 * One motion idea, used sparingly: when the editorial band enters the viewport,
 * its horizon line is drawn once from the centre. Product windows no longer
 * carry a drawn line — the approved photography brings its own light. Progressive enhancement — lines are visible without JavaScript,
...
export function useStorefrontMotion(path:string){useEffect(()=>{
 ...
 const targets=[...document.querySelectorAll<HTMLElement>('main.storefront .editorial, main.storefront .editorial-pair')].filter(el=>el.getBoundingClientRect().top>window.innerHeight*.9);
